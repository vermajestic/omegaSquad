...
export const TrackAnalysis: React.FC<TrackAnalysisProps> = ({
  ...
  return (
    <div className={`rounded-xl bg-[#111827] border border-slate-800 p-5 shadow-xl ${className}`}>
            ...
            <RechartsTooltip
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="bg-[#0a0f1e] border border-slate-700 p-2 rounded shadow-xl text-xs font-mono text-slate-200">
                      <div className="font-bold text-cyan-400">{data.time} UTC</div>
                      <div>Speed: <strong className="text-slate-100">{data.speed} kts</strong></div>
      ...
      {closest && (
        <div className="p-3 rounded-lg bg-amber-950/20 border border-amber-500/30 flex items-start gap-3">
          ...
          <div className="text-xs text-slate-300">
            ...
            <div className="text-slate-400 leading-snug">
              At <strong className="text-slate-200">{formatTime(closest.timestamp)}</strong>, vessel slowed down to 13.5 kts at coordinates{' '}
