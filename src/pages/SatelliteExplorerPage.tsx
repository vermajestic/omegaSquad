...
export const SatelliteExplorerPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'preprocessed':
        return 'bg-cyan-950/80 text-cyan-400 border border-cyan-800';
      default:
    ...
    <div className="space-y-6 pb-14">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        ...
        <Button
          ...
          className="text-xs shadow-[0_0_15px_rgba(6,182,212,0.3)]"
        >
      ...
      {/* Filter Toolbar */}
      <div className="p-4 rounded-xl bg-[#111827] border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {filteredScenes.map((scene) => (
          <div
            key={scene.id}
            className="rounded-xl bg-[#111827] border border-slate-800 hover:border-slate-700 p-4 shadow-xl flex flex-col justify-between transition group"
          >
            <div>
              {/* Header */}
              <div className="flex items-start justify-between gap-1 mb-2">
                <div>
                  <span className="text-xs font-mono font-bold text-cyan-400">{scene.id}</span>
                  <div className="text-sm font-bold text-white mt-0.5">{scene.region}</div>
                </div>
                <span className={`text-[10px] px-2 py-0.5 rounded uppercase font-mono font-bold ${getStatusBadge(scene.processingStatus)}`}>
                  {scene.processingStatus}
                </span>
              </div>

              {/* Sensor Badge */}
              <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-[11px] text-slate-300 font-mono mb-3">
                <Satellite className="w-3 h-3 text-cyan-400" />
                <span>{scene.satellite} • {scene.sensor}</span>
              </div>

              {/* Metadata */}
              <div className="space-y-1.5 text-xs text-slate-400 pt-2 border-t border-slate-800/80 font-mono">
                <div className="flex justify-between">
                  <span>Acquisition:</span>
                  <span className="text-slate-200">{formatDate(scene.acquisitionDate)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-200">{formatArea(scene.coverageArea)}</span>
                </div>
            ...
            <div className="pt-4 mt-3 border-t border-slate-800">
              ...
              >
                ...
                <ArrowRight className="w-3.5 h-3.5 ml-1 text-slate-400 group-hover:text-cyan-400" />
              </Button>
            </div>
          </div>
        ))}
